...
\subsection{Companion Drone Puzzle Objects}

These objects will only be accessible to the companion drone and commonly in puzzle areas.

\subsubsection{Battery}

The battery is a movable object that delivers power to powered objects such as hatches. It can fall and be pushed by the companion drone. If it comes in contact with a Power Line, it will deliver power to it and activate anything connected to the Power Line. The battery comes in two states: drained and full (for now). Chrono Restore can fully charge the battery or completely drain it.

\subsubsection{Stalactite}

Stalactites objects that block the path of movable objects and the companion drone. When they are on the ground, the companion drone can use Chrono Restore to revert it back to the ceiling, opening the path it was blocking. 

\subsubsection{Debris}

Debris comes in many looks where the parts inside the debris resemble something broken. Chrono Restore can revert the debris back to its original form, effectively opening a blocked path and restoring an arbitrary object into an position. If an object is already in the way of the new object to be restored, the restoration is cancelled and the object becomes debris again. 

\subsubsection{Hatch (Powered)}

Hatches are doors that can be opened/closed when powered/not powered. They can be placed vertically and horizontally.

\subsubsection{Power Line}

Power Lines transfer power from a power source to an object that consumes power. 
...
The companion drone can push small objects that are hanging off a  roof or on ground.
