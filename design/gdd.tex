...
\subsubsection{Combat Areas}

Combat areas contain several enemy spawners and usually feature lots of Key Objects that help the player during combat. Along with that, there are also hazards that can both put the enemy and the player at a disadvantage. 
...
Despite the fact that the player can make a checkpoint wherever they want using Chrono Drift, there will be "automatic" checkpoints placed throughout the level with extra functionality. These checkpoints are placed at the beginning of a level and can only serve as a checkpoint when the player quits the level to save their progress to play later. Additionally, this type of checkpoint allows the player to use scrap to instantly replenish a certain amount of ammo, health and power-up charge. 

\subsubsection{Transitional Areas}

Transitional areas are more dangerous areas to traverse through and serve as a gateway to another large combat area. These areas can be dangerous due to several reasons, such as being tighter, containing more hazards or requiring platforming. 

\subsubsection{Objective Placement and Types}

\paragraph{Planting Bomb at Objective}

This objective type will be most common for the levels. At the very end of a level, there is a large Fabricator that the player must plant an explosive at (simply by pressing a button). This starts a timer before the explosive detonates so that the player has room to escape the level before the detonation. At the objective point, there will be enemies guarding it. Depending on the level's design and difficulty, there will be either regular enemies (small minions) or mini-bosses guarding it, so the player must defeat them before proceeding. Escaping the level successfully (returning to the start of the level) within the timer means completing the level. This means the player must briefly learn the randomly generated level to successfully escape.

\paragraph{Boss Fights}

Some levels have a boss at the center which the player must defeat in order to complete the level. Each boss design will be unique and have a relevance to the story. 
